...
export default function App(disabled, sub_id) {

    const [subject_id, setSubId] = useState(sub_id)
    ...
    const save = async (eid) => {
        try {
            ...
            const response = await fetch("http://localhost:8080/subjects/" + String(eid), {
                method: 'POST',
                headers: {
        ...
        return (
            <Box sx={{ flexGrow: 1 }} margin="10px" padding="10px">
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={6}>
                        <Box>
                            <TextField disabled={false} fullWidth
                ...
                <Grid container spacing={2} visibility={!disabled}>
                    <Grid item xs={6}>
